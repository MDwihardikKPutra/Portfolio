...
void main(){
  ...
  // Tint with user custom smokeColor
...
class Renderer {
  ...
  constructor(canvas: HTMLCanvasElement, fragmentSource: string) {
    ...
    if (!gl) {
      throw new Error("WebGL2 is not supported by your browser/device.");
    }
    this.gl = gl;
  ...
  updateScale() {
    ...
    // OPTIMIZATION: Measure parent container size instead of window to prevent squishing
    const rect = this.canvas.parentNode
      ? (this.canvas.parentNode as HTMLElement).getBoundingClientRect()
...
export const SmokeBackground: React.FC<SmokeBackgroundProps> = memo(({ 
  smokeColor = "#808080"
}) => {
    ...
    useEffect(() => {
        ...
        try {
          ...
          console.error("WebGL2 is not supported:", e);
          return;
        }
